...
document.querySelectorAll('.regform').forEach((el2) => observer.observe(el2));

const observer2 = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
        console.log(entry)
        if (entry.isIntersecting) {
            entry.target.classList.add('fcshow');
        }
    });
});

document.querySelectorAll('.footer-col').forEach((el2) => observer.observe(el2));
...
F1("fname", "Naman");
F1("lname", "Kabra");
F1("email", "kabrasahab@gmail.com");
F1("phone", "9857518453");
F1("aadhar", "754869587812");
F1("pass", "supersecurepassword");
